...
def test_mixed_options_with_multi_leg_and_naked():
    ...
    test_expiry_spy = (datetime.now() + timedelta(days=90)).strftime("%Y-%m-%d")
    test_expiry_aapl = (datetime.now() + timedelta(days=60)).strftime("%Y-%m-%d")
    test_expiry_msft = (datetime.now() + timedelta(days=120)).strftime("%Y-%m-%d")
